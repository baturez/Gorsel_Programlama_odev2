...
namespace MauiApp1.Models
{
    public class KurlarViewModel : BaseViewModel
    {
        ...
        private async Task GetKurlarAsync()
        {
                ...
                if (!string.IsNullOrEmpty(jsondata))
                {
                    ...
                    if (data != null)
                    {
                        foreach (var item in data)
                        {
                            ...
                            double.TryParse(
                                kurDetay?.Deðiþim?.Replace("%", "").Replace(",", "."),
                            ...
                            KurlarListesi.Add(new KurModel
                            {
                                Tur = item.Key,
                                Alis = kurDetay?.Alýþ,
                                Satis = kurDetay?.Satýþ,
                                Fark = kurDetay?.Deðiþim,
            ...
            {
                await Application.Current.MainPage.DisplayAlert("Hata", $"Veriler alýnýrken bir hata oluþtu: {ex.Message}", "Tamam");
            }
            finally
            {
                IsBusy = false;
